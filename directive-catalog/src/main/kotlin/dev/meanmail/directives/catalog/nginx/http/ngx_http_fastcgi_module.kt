...
val fastcgiBuffering = ToggleDirective(
    "fastcgi_buffering",
    "Controls whether buffering of FastCGI server responses is enabled",
    enabled = true,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiCacheLock = ToggleDirective(
    "fastcgi_cache_lock",
    "Enables or disables locking mechanism to prevent multiple simultaneous requests for the same content",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiConvertVariables = ToggleDirective(
    "fastcgi_convert_variables",
    "Controls the conversion of variables in FastCGI request parameters",
    enabled = true,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiForceRanges = ToggleDirective(
    ...
    "Enables or disables forcing of byte-range requests for FastCGI responses",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiIgnoreClientAbort = ToggleDirective(
    ...
    "Controls whether to continue processing requests if the client closes the connection",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiInterceptErrors = ToggleDirective(
    ...
    "Controls whether NGINX will process 4xx and 5xx error responses from the FastCGI server",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiKeepConn = ToggleDirective(
    "fastcgi_keep_conn",
    "Controls whether to keep the connection with the FastCGI server open after request processing",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiParamLongValue = ToggleDirective(
    "fastcgi_param_long_value",
    "Controls the handling of long parameter values in FastCGI requests",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiPassRequestBody = ToggleDirective(
    ...
    "Controls whether the client request body is passed to the FastCGI server",
    enabled = true,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiPassRequestHeaders = ToggleDirective(
    ...
    "Controls whether client request headers are passed to the FastCGI server",
    enabled = true,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiRequestBuffering = ToggleDirective(
    ...
    "Enables or disables buffering of the client request body for FastCGI requests",
    enabled = true,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
...
val fastcgiSocketKeepalive = ToggleDirective(
    ...
    "Controls the use of the SO_KEEPALIVE socket option for maintaining persistent connections with FastCGI servers",
    enabled = false,
    context = listOf(http, server, location),
    module = ngx_http_fastcgi_module
)
